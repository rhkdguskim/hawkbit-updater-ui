...
const ApprovalPolicyCard: React.FC<{ isEditMode: boolean }> = ({ isEditMode }) => {
    ...
    const renderConditionInput = (rule: ApprovalRule) => {
        if (!isEditMode) {
            switch (rule.type) {
                case 'target_count':
                    return <Text strong>{rule.condition.threshold} {t('common:targets', 'targets')}</Text>;
                case 'tag':
                    return <Text strong>"{rule.condition.tag}"</Text>;
                case 'target_type':
                    return <Text strong>{rule.condition.targetType}</Text>;
                case 'time_range':
                    return <Text strong>{rule.condition.start} - {rule.condition.end}</Text>;
                default:
                    return null;
            }
        }
        ...
        switch (rule.type) {
            case 'target_count':
                return (
                    <InputNumber
                        ...
                        onChange={(val) => updateRule(rule.id, { ...rule.condition, threshold: val || 0 })}
                        min={0}
                        style={{ width: 120 }}
                    />
                );
            ...
            case 'target_type':
                return (
                    <Input
                        value={rule.condition.targetType}
            ...
            case 'time_range':
                return (
                    <Space>
                        <TimePicker
                            value={dayjs(rule.condition.start, 'HH:mm')}
                            format="HH:mm"
                            onChange={(time) => updateRule(rule.id, { ...rule.condition, start: time?.format('HH:mm') || '00:00' })}
                            style={{ width: 100 }}
                        />
                        <Text>-</Text>
                        <TimePicker
                            value={dayjs(rule.condition.end, 'HH:mm')}
                            format="HH:mm"
                            onChange={(time) => updateRule(rule.id, { ...rule.condition, end: time?.format('HH:mm') || '00:00' })}
                            style={{ width: 100 }}
                        />
                    </Space>
                );
